...
class DocsSpecs:
    def __init__(self, file = None, dest_bucket=None, dest_path=None, extractor=None, writer=None):
        ...
        self.writer = writer
        self.types = {'pdf' : {'bucket_ocr':'ngcs-chat-poc-temp-bucket', 
                               'bucket_txt' :  'ngcs-chat-poc-txt-bucket'}
                      ,
                      'docx': 'ngcs-chat-poc-txt-bucket',
                      'bq' : {'project_id':'ai-services-401511',
                              'dataset_id': 'chat',
    ...
    def check_file_type(self):
        if self.type == 'pdf':
            ...
            if ('ocrmypdf' in metadata.get('/Creator', '')):
                ...
                dest_bucket = self.docs_specs.types.get('pdf')['bucket_txt']
                extractor = PDFTextExtractor(self.path.split('/')[-1])
                writer = WriteTextToBucket(dest_bucket, dest_path)
            else:
                dest_bucket = self.docs_specs.types.get('pdf')['bucket_ocr']
            ...
            dest_bucket = self.docs_specs.types.get('docx')
            dest_path = str(Path(self.path.split('/')[-1]).with_suffix('.txt'))
            writer = WriteObjectToBucket(dest_bucket,dest_path)
...
class DocumentFlow:
    ...
    def process(self):
        ...
        #get content
        project_id = specs.types.get('bq')['project_id']
        dataset_id = specs.types.get('bq')['dataset_id']
        table_id = specs.types.get('bq')['table_id']
        ...
        try:
            ...
            bq_updater.update_bigquery_row(project_id, dataset_id, table_id,column_updates, condition)
        except Exception as e:
            print(e)
