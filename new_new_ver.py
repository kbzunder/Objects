...
class DocsSpecs:
    def __init__(self, file = None, dest_bucket=None, dest_path=None):
        self.file = file,
        self.dest_bucket = dest_bucket,
        self.dest_path = dest_path
        
    
    def set_parameters(self,file, dest_bucket, dest_path):
        self.file = file,
        self.dest_bucket = dest_bucket,
        self.dest_path = dest_path
                 
    def get_parameters(self):
        return self.file, self.dest_bucket, self.dest_path
                 
                 
class DocumentCassifier:
    def __init__(self, path, DownloadFileFromBucket, DocsSpecs):
        self.path = path
    ...
    def check_file_type(self):
        ...
        if self.type == 'pdf':
            ...
            dest_path = str(Path(self.path.split('/')[-1]).with_suffix('.txt'))
            writer = WriteObjectToBucket(dest_bucket,dest_path)
        self.docs_specs.set_parameters(self.file,dest_bucket, dest_path)
        return  extractor, writer, self.docs_specs
...
class FileProcessor:
    def __init__(self, bucket, name, DocumentCassifier, BigqueryUpdater):
        ...
        self.classifier = DocumentCassifier(self.path, DownloadFileFromBucket, DocsSpecs)
        self.extractor, self.writer, self.doc_specs = self.classifier.check_file_type()
        self.file, self.dest_bucket, self.dest_path = self.doc_specs.get_parameters()
        self.content = self.extractor.text_extract() if self.extractor else self.file
        self.column_updates = {'Path' : self.dest_path,
